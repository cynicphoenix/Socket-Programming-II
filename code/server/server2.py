...
def upload(client_socket, host_ip, host_port, client_ip, client_port):
    ...
    client_socket.send(('Filename & Filesize Recieved by server ' + host + ' : ' + port + '.' ).encode())
    ...
    while(count != int(total_packet)):     
        ...
        print('Recieved from client ' + client_ip + ' : ' + str(client_port) + ' '+ str(count) + '/' + total_packet + '.')

        host_socket.sendall(bytes_read)
        print(host_socket.recv(BUFFER_SIZE).decode())
